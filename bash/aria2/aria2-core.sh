...
apt-get install -y screen crontab
...
# Directory, Files, & URL Parameters
url_aria2c="https://github.com/aria2/aria2.git"
dir_aria2c_install="/root/aria2c"
dir_aria2c_config="/root/.aria2"
file_aria2c_binary="/usr/bin/aria2c"
file_aria2c_config="${dir_aria2c_config}/aria2.conf"
file_aria2c_session="${dir_aria2c_config}/aria2.session"

# Cleanup system before Install Aria2c
rm -rf "${file_aria2c_binary}"
rm -rf "${dir_aria2c_install}"
rm -rf "${dir_aria2c_config}"

# Get Aria2c Source
mkdir -p "${dir_aria2c_install}"
git clone "${url_aria2c}"

# Install Aria2c
cd "${dir_aria2c_install}" || (echo -e "${dir_aria2c_install} : Directory not found" && exit)
autoreconf -i
./configure
make
mv "${dir_aria2c_install}/src/aria2c" "${file_aria2c_binary}"
chmod +x "${file_aria2c_binary}"
rm -rf "${dir_aria2c_install}"

# Configuring Aria2c
rpc_token=$(LC_CTYPE=C tr -dc '[:lower:]' < /dev/urandom | head -c 5)
rpc_port=$(shuf -i49152-65535 -n1)
mkdir -p "${dir_aria2c_config}"
touch "${file_aria2c_session}"
cat >"${file_aria2c_config}" <<-EOF
### Basic ###
# The directory to store the downloaded file.
dir=/var/www/html/aria2/dl
...
rpc-listen-port=port_rpcku
# Set RPC secret authorization token.
rpc-secret=tokenkoooo
...
EOF
sed -i '/^# /d' "${file_aria2c_config}"
sed -i "s/tokenkoooo/${rpc_token}/g" "${file_aria2c_config}"
sed -i "s/port_rpcku/${rpc_port}/g" "${file_aria2c_config}"

# Set onboot by Crontab
crontab -l > /tmp/cron.txt
sed -i '/aria2d/d' /tmp/cron.txt
sed -i '$ i\@reboot screen -dmS aria2d aria2c --conf-path=/root/.aria2/aria2.conf >/dev/null' /tmp/cron.txt
